...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['CRONUS_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['CRONUS_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Cronus API.
 */
export class Cronus {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['CRONUS_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('CRONUS_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.CronusError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Cronus client with an apiKey option, like new Cronus({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Cronus.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('CRONUS_LOG'), "process.env['CRONUS_LOG']", this) ??
      defaultLogLevel;
  ...
  static CronusError = Errors.CronusError;
...
export declare namespace Cronus {
